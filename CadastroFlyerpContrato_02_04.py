...
# URL do webservice
url = "https://jttelecom.flyerp.com.br/apis/CadastrarContrato"

# Bearer Token para autenticação
bearer_token = "9d3d2669-7c79-400e-8f45-f889e23d0736"
...
csv_file_path = r"D:/FlyERP/Dev/Automação de cadastros/Pasta de Arquivos/Contratos_jttelecom_v3.csv"
erro_csv_file_path = r"D:/FlyERP/Dev/Automação de cadastros/Pasta de Arquivos/registros_falhados_Contratos_jttelecom.csv"
...
with open(erro_csv_file_path, mode='w', newline='', encoding='utf-8-sig') as erro_file:
    ...
    erro_writer.writerow(['cnpj_cpf_cliente', 'referencia', 'referencia_busca_cliente', 'data_primeiro_vencimento', 
                ...
                try:
                    ...
                    for linha in linhas:
                        servico = {
                            ...
                            # "quantidade": safe_int(linha.get('servicos.quantidade', 1)),
                            "quantidade": int(format_decimal(linha.get('servicos.quantidade', '1'))),
                        ...
                        if linha.get('servicos.codigo_politica_comissao'):
                            servico["comissao"] = {
                                "codigo_politica_comissao": safe_int(linha.get('servicos.codigo_politica_comissao', 0)),
                                "codigos_vendedores": [safe_int(x) for x in linha.get('servicos.codigos_vendedores', '').split(',') if x],
                                "comissoes": [float(x) for x in linha.get('servicos.comissoes', '').split(',') if x]
                            }
                    ...
                    payload = {
                        ...
                        "dia_vencimento": safe_int(contrato_data.get('dia_vencimento', 1)),
                        "descricao": contrato_data.get('descricao', ''),
                        "numero": contrato_data.get('numero', ''),                        
                        "codigo_vendedor": safe_int(contrato_data.get('codigo_vendedor', 0)),
                        ...
                        "recorrencia": safe_int(contrato_data.get('recorrencia', 1)),
                        "tipo_faturamento": safe_int(contrato_data.get('tipo_faturamento', 0)),
                        "indice_reajuste": safe_int(contrato_data.get('indice_reajuste', 0)),
                        "codigo_banco_conta": safe_int(contrato_data.get('codigo_banco_conta', 0)),
                        "codigo_forma_pagamento": safe_int(contrato_data.get('codigo_forma_pagamento', 0)),
                        "codigo_plano_contas": safe_int(contrato_data.get('codigo_plano_contas', 7)),
                        "codigo_condicao_pagamento": safe_int(contrato_data.get('codigo_condicao_pagamento', 0)),
                        ...
                        "tipo_desconto_boleto_ate_vencimento": safe_int(contrato_data.get('tipo_desconto_boleto_ate_vencimento', 0)),
                    ...
                    if 'cliente.cnpj_cpf' in contrato_data:
                        payload["cliente"] = {
                            ...
                            "codigo_vendedor": safe_int(contrato_data.get('cliente.codigo_vendedor', 0)),
                            "codigo_ramo_atividade": safe_int(contrato_data.get('cliente.codigo_ramo_atividade', 0)),
